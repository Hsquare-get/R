# TIL0929 (웹사이트 테이블, ggplot2 패키지-고급시각화, ggmap 패키지-지도시각화)

## (1) 웹사이트의 테이블 한번에 읽어오기

```R
# 정적 웹사이트의 테이블을 한번에 읽어오기
library(XML)
library(rvest)
...
Sys.setlocale("LC_ALL", "English") # 일단 로케일을 영어로 변경해서 가져오기(한글 깨지는 것을 처리하기 위해)
games_table <- readHTMLTable(elem_html, header = T, stringsAsFactors = FALSE)[[1]]
Sys.setlocale() # 기본 로케일로 다시 변경
...
> 층층이 쌓는 **layering 방식**으로 시각화하는 패키지. 동일한 문법으로 다양한 그래프를 그릴 수 있고, 비교적 쉽게 높은 수준의 그래프를 그릴 수 있는 것이 장점이다.
>
> 
>
...
> **Data**
>
> 그래프를 그리려는 데이터로 구조는 데이터 프레임이고 데이터의 기록 방식은 long-format에 기반한 **tidy** **data**여야 한다.
>
> **(Aesthetic)Mapping**
>
...
![ggolot2 layer](https://user-images.githubusercontent.com/64063767/94499493-d1b9be80-0237-11eb-8f03-d4755b0d8938.png)
...
mpg <- as.data.frame(ggplot2::mpg); str(mpg)
...
sketch + geom_point(aes(color=drv)) # 구동방식별로 색을 지정
...
library(xlsx)
classDF <- read.xlsx("data/data.xlsx", 1, encoding="UTF-8") # 첫번째 시트 읽기
str(classDF)
View(classDF)

bar_data <- classDF[3]; bar_data
str(bar_data)
...
coSalesDF <- data.frame(company, year, sales); coSalesDF
str(coSalesDF)
...
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, color=company)) + scale_color_grey()
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, color=company)) + scale_color_hue()
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, color=company)) + scale_color_manual(values=c("orange", "green"))
# 선의 종류 : 0=blank, 1=solid, 2=dashed, 3=dotted, 4=dotdash, 5=longdash, 6=twodash
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, color=company), linetype=3)
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, color=company), linetype="dotdash")
# 점의 종류와 색상
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, color=company)) + geom_point(size=2, shape=5)
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, color=company)) + geom_point(size=2, shape='가')
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, color=company)) + geom_point(size=2, shape='가', color="Red")
```
...
ggplot(Cars93, aes(x=Weight, y=MPG.highway)) + geom_point(shape=21, size=6, color="blue")
ggplot(Cars93, aes(x=Weight, y=MPG.highway)) + geom_point(shape=21, size=6, fill="blue") 
ggplot(Cars93, aes(x=Weight, y=MPG.highway)) + geom_point(shape=21, size=6, fill="blue", color="pink") 
ggplot(Cars93, aes(x=Weight, y=MPG.highway)) + geom_point(color="grey", shape=21, size=6) 
ggplot(Cars93, aes(x=Weight, y=MPG.highway, fill=Price)) + geom_point(color="grey", shape=21, size=6) 
ggplot(Cars93, aes(x=Weight, y=MPG.highway)) + geom_point(color="grey", shape=21, size=6, aes(fill=Price)) 
ggplot(Cars93, aes(x=Weight, y=MPG.highway, fill=Cylinders)) +geom_point(color="grey", shape=21, size=6)
ggplot(Cars93, aes(x=Weight, y=MPG.highway, fill=Cylinders)) + geom_point(color="grey", shape=21, size=6) + 
  scale_fill_brewer(palette="Oranges") # "Reds", "Blues"
```
...
ggplot(data=w, aes(x=year, y=weight)) + geom_bar() # 개수를 세서 막대그래프를 그리는 것이 기본이기 때문에 에러발생
...
#행 설정
ggplot(mpg, aes(x=displ, y=hwy, color=manufacturer)) + geom_point() + guides(color=guide_legend(nrow=6))
#열 설정
ggplot(mpg, aes(x=displ, y=hwy, color=manufacturer)) + geom_point() + scale_fill_brewer(palette="Reds") + guides(color=guide_legend(ncol=2))
# 범례 항목들 역순으로
ggplot(mpg, aes(x=displ, y=hwy, color=manufacturer)) + geom_point() + guides(color=guide_legend(reverse=TRUE))
# 범례 없애기
ggplot(mpg, aes(x=displ, y=hwy, color=manufacturer)) + geom_point() + guides(color=F)
```



```R
### 레이아웃을 나눠 여러 그래프를 나타내기 ###
...
library(plotly)
# ggplot으로 그래프 만들기
p <- ggplot(data=mpg, aes(x=displ, y=hwy, col=drv)) + geom_point(); p
...
sales_df <- read.xlsx("data/data.xlsx", 2, encoding="UTF-8"); sales_df

# 트리맵 그리기 (product를 기준으로 먼저 나눈)
# index에 표현하고 싶은 계층 순서대로 벡터로 생성
# product, region 순으로 벡터를 지정함으로써 product가 region보다 더 상위로 구분이 됨
treemap(sales_df, vSize="saleAmt", index=c("product", "region"), title="A기업 판매현황")
# 트리맵 그리기(region을 기준으로 먼저 나눈)
...
lon <- 126.9221322; lat <- 37.5268831
cen <- c(lon,lat)
mk <- data.frame(lon=lon, lat=lat)

map <- get_googlemap(center=cen, maptype="roadmap", zoom=1, marker=mk); Sys.sleep(1);
ggmap(map)
map <- get_googlemap(center=cen, maptype="roadmap", zoom=5, marker=mk); Sys.sleep(1);
ggmap(map)
map <- get_googlemap(center=cen, maptype="roadmap", zoom=10, marker=mk); Sys.sleep(1);
ggmap(map)
map <- get_googlemap(center=cen, maptype="roadmap", zoom=15, marker=mk); Sys.sleep(1);
ggmap(map)
map <- get_googlemap(center=cen, maptype="satellite", zoom=16, marker=mk); Sys.sleep(1);
ggmap(map)
map <- get_googlemap(center=cen, maptype="terrain", zoom=8, marker=mk); Sys.sleep(1);
ggmap(map)
map <- get_googlemap(center=cen, maptype="terrain", zoom=12, marker=mk); Sys.sleep(1);
ggmap(map)
map <- get_googlemap(center=cen, maptype="hybrid", zoom=16, marker=mk); Sys.sleep(1);
ggmap(map) + labs(title="테스트", x="경도", y="위도")

map <- get_map(location=cen, maptype="toner", zoom=12, marker=mk, source="google"); ggmap(map)
map <- get_map(location=cen, maptype="watercolor", zoom=12, marker=mk, source="stamen"); ggmap(map)
map <- get_map(location=cen, maptype="terrain-background", zoom=12, marker=mk, source="stamen"); ggmap(map)
map <- get_map(location=cen, maptype="roadmap", zoom=12, marker=mk, source="stamen"); ggmap(map)
map <- get_map(location=cen, maptype="terrain", zoom=12, marker=mk, source="stamen"); ggmap(map)
...
geocode('Seoul', source = 'google', output = 'latlona') # lat lon address
...
# 다음 행은 한번만 수행
station_lonlat = mutate_geocode(station_df, station_list, source = 'google'); station_lonlat
...
seoul_map + geom_point(data = station_lonlat, aes(x=lon, y=lat), color='green', size = 4)
```
...
leaflet() %>% addTiles() 
# 지도 배경에 센터 설정하기
map0 <- leaflet() %>% setView(lng = seoul_lonlat$lon, lat = seoul_lonlat$lat, zoom = 16); map0
# 지도 배경에 센터 설정하고 타일깔기
map1 <- map0 %>% addTiles() ; map1

mk <- multi_lonlat
lon <- mk$lon; lat <- mk$lat
