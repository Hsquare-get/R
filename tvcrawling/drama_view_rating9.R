...
# MBC 십시일반 날짜별 시청률 from 위키피디아
url = "https://ko.wikipedia.org/wiki/%EC%8B%AD%EC%8B%9C%EC%9D%BC%EB%B0%98_(%EB%93%9C%EB%9D%BC%EB%A7%88)"
html <- read_html(url) # XML::read_html
parse <- htmlParse(html) # XML::htmlParse
wiki_table <- getNodeSet(parse, "//table")[[2]]
tenone_table <- readHTMLTable(wiki_table); tenone_table
str(tenone_table); View(tenone_table) # df

# 방영일자별 시청률 가져오기
d <- tenone_table$V2[seq(4,18, 2)]; d
d <- gsub("[가-힣]", "", d); d
d <- as.Date(d, "%m %d"); d

#tenone_table[[4]][4]; tenone_table[[2]][5]
#tenone_table[[4]][6]; tenone_table[[2]][7]
#tenone_table[[4]][8]; tenone_table[[2]][9]
#...
#tenone_table[[4]][34]; tenone_table[[2]][35]

vr <- c()
for (i in seq(4,18, 2)){
  vr <- c(vr, c(tenone_table[[4]][i]))
  vr <- c(vr, c(tenone_table[[2]][i+1]))
}
vr
vr <- gsub("%", "", vr)
vr <- as.numeric(vr)
...
tenone_dataset <- data.frame(day=date, view_rating=vr); str(tenone_dataset) # 초기 시청률 df
View(tenone_dataset)


# 평일 데이터만 남기기
day <- as.Date(tenone_dataset$day, "%Y%m%d"); str(day)
yoil <- format(day, "%a"); str(yoil); tail(yoil)

tenone_dataset <- tenone_dataset %>% mutate(yoil=yoil); View(tenone_dataset)
...
tenone_dataset <- cbind(tenone_dataset,
...
wheather_data <- wheather_data %>% filter(time >= strptime("2020-07-22", "%Y-%m-%d") & time < strptime("2020-08-14", "%Y-%m-%d"))
...
tenone_dataset <- cbind(tenone_dataset, temp=temp, rain=rain)
View(tenone_dataset)

# 주말여부(휴일) 변수 추가
tenone_dataset <- tenone_dataset %>% mutate(weekend=ifelse(yoil=="토" | yoil=="일", "Y", "N"))

# 일일유동인구(서울 지하철 승하차인원수) & 이전회차 시청률(or 전작품 평균시청률)
tenone_dataset <- tenone_dataset %>% mutate(floating_population=NA, pre_view_rating=NA)


tenone_dataset <- tenone_dataset %>% select(broadcast_station, program, day, yoil, weekend, time, story_step,
                                            temp, rain, floating_population, pre_view_rating, view_rating)

View(tenone_dataset)
getwd(); setwd("C:/Rexam/tvcrawling"); getwd()
write.csv(tenone_dataset, file="tenone.csv")
